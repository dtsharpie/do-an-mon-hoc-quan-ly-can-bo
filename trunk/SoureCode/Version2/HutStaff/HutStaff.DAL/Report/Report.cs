...
namespace HutStaff.DAL.Report
{
    public class Report
    {
        ...
        // Lay du lieu de lam bao cao loai 4.1: Báo cáo chất lượng cán bộ công chức chia theo lĩnh vực
        public static DataTable GetDataTableToReport_Type_4_1(string ma_dv, string diencb, string tt)
        {
            using (MainDB db = new MainDB())
            {
                return db.Execute("[sp_report_type_4_1]",
                    new string[] { "@ma_dv", "@dcb", "@tt" },
                    new object[] { ma_dv, diencb, tt });
            }
        }

        // Lay du lieu de lam bao cao loai 4.2: Báo cáo chất lượng cán bộ công chức chia theo đơn vị
        public static DataTable GetDataTableToReport_Type_4_2(string ma_dv, string diencb, string tt)
        {
            using (MainDB db = new MainDB())
            {
                return db.Execute("[sp_report_type_4_2]",
                    new string[] { "@ma_dv", "@dcb", "@tt" },
                    new object[] { ma_dv, diencb, tt });
            }
        }
